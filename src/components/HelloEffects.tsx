...
function AppleHelloEnglishEffect({
  ...
  const container = {
    ...
    visible: {
      opacity: 1,
      transition: { staggerChildren: 0.05, delayChildren: 0.2 * speed },
    },
  };
  ...
  React.useEffect(() => {
    const holdTime = 1200;
    const totalTime = (0.2 * speed * 1000) + (text.length * 0.05 * 1000) + 500 + holdTime;
  ...
  }, [onAnimationComplete, speed, text.length]);

  return (
    <motion.div
      className={cn("flex flex-wrap items-center justify-center px-4 text-center", className)}
      variants={container}
      initial="hidden"
      animate="visible"
